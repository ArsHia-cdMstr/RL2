...
def policy_iteration(env, custom_map, max_ittr=30, theta=0.01, discount_factor=0.9):
    ...
    while not policy_stable and ittr < max_ittr:
        ...
        ittr += 1
    policy = conver_plicy(env, policy)
    return V, policy


def conver_plicy(env, policy):
    p = {}
    for s in range(env.observation_space.n):
        arr_list = plicy[s]
    ...
    for i_episode in range(num_episodes):
        ...
        while not done :
            if type(state) == tuple:
                ...
                values = list(plicy[state[0].values()])
            else:
                keys = list(policy[state].key())
...
if __name__ == "__main__":
    ...
    V, policy = policy_iteration(env, map, theta=0.0001, discounted_factor=0.99)
